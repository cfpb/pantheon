 ...
 *
 *  dev-dash - v0.1.0
 *  https://github.com/cfpb/dev-dash *  A public domain work of the Consumer Financial Protection Bureau
 */
/*!
...
}}),ngTranscludeDirective=ngDirective({restrict:"EAC",link:function($scope,$element,$attrs,controller,$transclude){if(!$transclude)throw minErr("ngTransclude")("orphan","Illegal use of ngTransclude directive in the template! No parent directive that requires a transclusion found. Element: {0}",startingTag($element));$transclude(function(clone){$element.empty(),$element.append(clone)})}}),scriptDirective=["$templateCache",function($templateCache){return{restrict:"E",terminal:!0,compile:function(element,attr){if("text/ng-template"==attr.type){var templateUrl=attr.id,text=element[0].text;$templateCache.put(templateUrl,text)}}}}],ngOptionsMinErr=minErr("ngOptions"),ngOptionsDirective=valueFn({restrict:"A",terminal:!0}),selectDirective=["$compile","$parse",function($compile,$parse){var NG_OPTIONS_REGEXP=/^\s*([\s\S]+?)(?:\s+as\s+([\s\S]+?))?(?:\s+group\s+by\s+([\s\S]+?))?\s+for\s+(?:([\$\w][\$\w]*)|(?:\(\s*([\$\w][\$\w]*)\s*,\s*([\$\w][\$\w]*)\s*\)))\s+in\s+([\s\S]+?)(?:\s+track\s+by\s+([\s\S]+?))?$/,nullModelCtrl={$setViewValue:noop};return{restrict:"E",require:["select","?ngModel"],controller:["$element","$scope","$attrs",function($element,$scope,$attrs){var nullOption,unknownOption,self=this,optionsMap={},ngModelCtrl=nullModelCtrl;self.databound=$attrs.ngModel,self.init=function(ngModelCtrl_,nullOption_,unknownOption_){ngModelCtrl=ngModelCtrl_,nullOption=nullOption_,unknownOption=unknownOption_},self.addOption=function(value,element){assertNotHasOwnProperty(value,'"option value"'),optionsMap[value]=!0,ngModelCtrl.$viewValue==value&&($element.val(value),unknownOption.parent()&&unknownOption.remove()),element&&element[0].hasAttribute("selected")&&(element[0].selected=!0)},self.removeOption=function(value){this.hasOption(value)&&(delete optionsMap[value],ngModelCtrl.$viewValue===value&&this.renderUnknownOption(value))},self.renderUnknownOption=function(val){var unknownVal="? "+hashKey(val)+" ?";unknownOption.val(unknownVal),$element.prepend(unknownOption),$element.val(unknownVal),unknownOption.prop("selected",!0)},self.hasOption=function(value){return optionsMap.hasOwnProperty(value)},$scope.$on("$destroy",function(){self.renderUnknownOption=noop})}],link:function(scope,element,attr,ctrls){function setupAsSingle(scope,selectElement,ngModelCtrl,selectCtrl){ngModelCtrl.$render=function(){var viewValue=ngModelCtrl.$viewValue;selectCtrl.hasOption(viewValue)?(unknownOption.parent()&&unknownOption.remove(),selectElement.val(viewValue),""===viewValue&&emptyOption.prop("selected",!0)):isUndefined(viewValue)&&emptyOption?selectElement.val(""):selectCtrl.renderUnknownOption(viewValue)},selectElement.on("change",function(){scope.$apply(function(){unknownOption.parent()&&unknownOption.remove(),ngModelCtrl.$setViewValue(selectElement.val())})})}function setupAsMultiple(scope,selectElement,ctrl){var lastView;ctrl.$render=function(){var items=new HashMap(ctrl.$viewValue);forEach(selectElement.find("option"),function(option){option.selected=isDefined(items.get(option.value))})},scope.$watch(function(){equals(lastView,ctrl.$viewValue)||(lastView=shallowCopy(ctrl.$viewValue),ctrl.$render())}),selectElement.on("change",function(){scope.$apply(function(){var array=[];forEach(selectElement.find("option"),function(option){option.selected&&array.push(option.value)}),ctrl.$setViewValue(array)})})}function setupAsOptions(scope,selectElement,ctrl){function callExpression(exprFn,key,value){return locals[valueName]=value,keyName&&(locals[keyName]=key),exprFn(scope,locals)}function selectionChanged(){scope.$apply(function(){var viewValue,collection=valuesFn(scope)||[];if(multiple)viewValue=[],forEach(selectElement.val(),function(selectedKey){selectedKey=trackFn?trackKeysCache[selectedKey]:selectedKey,viewValue.push(getViewValue(selectedKey,collection[selectedKey]))});else{var selectedKey=trackFn?trackKeysCache[selectElement.val()]:selectElement.val();viewValue=getViewValue(selectedKey,collection[selectedKey])}ctrl.$setViewValue(viewValue),render()})}function getViewValue(key,value){if("?"===key)return undefined;if(""===key)return null;var viewValueFn=selectAsFn?selectAsFn:valueFn;return callExpression(viewValueFn,key,value)}function getLabels(){var toDisplay,values=valuesFn(scope);if(values&&isArray(values)){toDisplay=new Array(values.length);for(var i=0,ii=values.length;ii>i;i++)toDisplay[i]=callExpression(displayFn,i,values[i]);return toDisplay}if(values){toDisplay={};for(var prop in values)values.hasOwnProperty(prop)&&(toDisplay[prop]=callExpression(displayFn,prop,values[prop]))}return toDisplay}function createIsSelectedFn(viewValue){var selectedSet;if(multiple)if(trackFn&&isArray(viewValue)){selectedSet=new HashMap([]);for(var trackIndex=0;trackIndex<viewValue.length;trackIndex++)selectedSet.put(callExpression(trackFn,null,viewValue[trackIndex]),!0)}else selectedSet=new HashMap(viewValue);else trackFn&&(viewValue=callExpression(trackFn,null,viewValue));return function(key,value){var compareValueFn;return compareValueFn=trackFn?trackFn:selectAsFn?selectAsFn:valueFn,multiple?isDefined(selectedSet.remove(callExpression(compareValueFn,key,value))):viewValue===callExpression(compareValueFn,key,value)}}function scheduleRendering(){renderScheduled||(scope.$$postDigest(render),renderScheduled=!0)}function updateLabelMap(labelMap,label,added){labelMap[label]=labelMap[label]||0,labelMap[label]+=added?1:-1}function render(){renderScheduled=!1;var optionGroupName,optionGroup,option,existingParent,existingOptions,existingOption,key,value,groupLength,length,groupIndex,index,selected,lastElement,element,label,optionId,optionGroups={"":[]},optionGroupNames=[""],viewValue=ctrl.$viewValue,values=valuesFn(scope)||[],keys=keyName?sortedKeys(values):values,labelMap={},isSelected=createIsSelectedFn(viewValue),anySelected=!1;for(trackKeysCache={},index=0;length=keys.length,length>index;index++)key=index,keyName&&(key=keys[index],"$"===key.charAt(0))||(value=values[key],optionGroupName=callExpression(groupByFn,key,value)||"",(optionGroup=optionGroups[optionGroupName])||(optionGroup=optionGroups[optionGroupName]=[],optionGroupNames.push(optionGroupName)),selected=isSelected(key,value),anySelected=anySelected||selected,label=callExpression(displayFn,key,value),label=isDefined(label)?label:"",optionId=trackFn?trackFn(scope,locals):keyName?keys[index]:index,trackFn&&(trackKeysCache[optionId]=key),optionGroup.push({id:optionId,label:label,selected:selected}));for(multiple||(nullOption||null===viewValue?optionGroups[""].unshift({id:"",label:"",selected:!anySelected}):anySelected||optionGroups[""].unshift({id:"?",label:"",selected:!0})),groupIndex=0,groupLength=optionGroupNames.length;groupLength>groupIndex;groupIndex++){for(optionGroupName=optionGroupNames[groupIndex],optionGroup=optionGroups[optionGroupName],optionGroupsCache.length<=groupIndex?(existingParent={element:optGroupTemplate.clone().attr("label",optionGroupName),label:optionGroup.label},existingOptions=[existingParent],optionGroupsCache.push(existingOptions),selectElement.append(existingParent.element)):(existingOptions=optionGroupsCache[groupIndex],existingParent=existingOptions[0],existingParent.label!=optionGroupName&&existingParent.element.attr("label",existingParent.label=optionGroupName)),lastElement=null,index=0,length=optionGroup.length;length>index;index++)option=optionGroup[index],(existingOption=existingOptions[index+1])?(lastElement=existingOption.element,existingOption.label!==option.label&&(updateLabelMap(labelMap,existingOption.label,!1),updateLabelMap(labelMap,option.label,!0),lastElement.text(existingOption.label=option.label),lastElement.prop("label",existingOption.label)),existingOption.id!==option.id&&lastElement.val(existingOption.id=option.id),lastElement[0].selected!==option.selected&&(lastElement.prop("selected",existingOption.selected=option.selected),msie&&lastElement.prop("selected",existingOption.selected))):(""===option.id&&nullOption?element=nullOption:(element=optionTemplate.clone()).val(option.id).prop("selected",option.selected).attr("selected",option.selected).prop("label",option.label).text(option.label),existingOptions.push(existingOption={element:element,label:option.label,id:option.id,selected:option.selected}),updateLabelMap(labelMap,option.label,!0),lastElement?lastElement.after(element):existingParent.element.append(element),lastElement=element);for(index++;existingOptions.length>index;)option=existingOptions.pop(),updateLabelMap(labelMap,option.label,!1),option.element.remove()}for(;optionGroupsCache.length>groupIndex;){for(optionGroup=optionGroupsCache.pop(),index=1;index<optionGroup.length;++index)updateLabelMap(labelMap,optionGroup[index].label,!1);optionGroup[0].element.remove()}forEach(labelMap,function(count,label){count>0?selectCtrl.addOption(label):0>count&&selectCtrl.removeOption(label)})}var match;if(!(match=optionsExp.match(NG_OPTIONS_REGEXP)))throw ngOptionsMinErr("iexp","Expected expression in form of '_select_ (as _label_)? for (_key_,)?_value_ in _collection_' but got '{0}'. Element: {1}",optionsExp,startingTag(selectElement));var displayFn=$parse(match[2]||match[1]),valueName=match[4]||match[6],selectAs=/ as /.test(match[0])&&match[1],selectAsFn=selectAs?$parse(selectAs):null,keyName=match[5],groupByFn=$parse(match[3]||""),valueFn=$parse(match[2]?match[1]:valueName),valuesFn=$parse(match[7]),track=match[8],trackFn=track?$parse(match[8]):null,trackKeysCache={},optionGroupsCache=[[{element:selectElement,label:""}]],locals={};nullOption&&($compile(nullOption)(scope),nullOption.removeClass("ng-scope"),nullOption.remove()),selectElement.empty(),selectElement.on("change",selectionChanged),ctrl.$render=render,scope.$watchCollection(valuesFn,scheduleRendering),scope.$watchCollection(getLabels,scheduleRendering),multiple&&scope.$watchCollection(function(){return ctrl.$modelValue},scheduleRendering)}if(ctrls[1]){for(var emptyOption,selectCtrl=ctrls[0],ngModelCtrl=ctrls[1],multiple=attr.multiple,optionsExp=attr.ngOptions,nullOption=!1,renderScheduled=!1,optionTemplate=jqLite(document.createElement("option")),optGroupTemplate=jqLite(document.createElement("optgroup")),unknownOption=optionTemplate.clone(),i=0,children=element.children(),ii=children.length;ii>i;i++)if(""===children[i].value){emptyOption=nullOption=children.eq(i);break}selectCtrl.init(ngModelCtrl,nullOption,unknownOption),multiple&&(ngModelCtrl.$isEmpty=function(value){return!value||0===value.length}),optionsExp?setupAsOptions(scope,element,ngModelCtrl):multiple?setupAsMultiple(scope,element,ngModelCtrl):setupAsSingle(scope,element,ngModelCtrl,selectCtrl)}}}}],optionDirective=["$interpolate",function($interpolate){var nullSelectCtrl={addOption:noop,removeOption:noop};return{restrict:"E",priority:100,compile:function(element,attr){if(isUndefined(attr.value)){var interpolateFn=$interpolate(element.text(),!0);interpolateFn||attr.$set("value",element.text())}return function(scope,element,attr){var selectCtrlName="$selectController",parent=element.parent(),selectCtrl=parent.data(selectCtrlName)||parent.parent().data(selectCtrlName);selectCtrl&&selectCtrl.databound||(selectCtrl=nullSelectCtrl),interpolateFn?scope.$watch(interpolateFn,function(newVal,oldVal){attr.$set("value",newVal),oldVal!==newVal&&selectCtrl.removeOption(oldVal),selectCtrl.addOption(newVal,element)}):selectCtrl.addOption(attr.value,element),element.on("$destroy",function(){selectCtrl.removeOption(attr.value)})}}}}],styleDirective=valueFn({restrict:"E",terminal:!1});return window.angular.bootstrap?void console.log("WARNING: Tried to load angular more than once."):(bindJQuery(),publishExternalAPI(angular),void jqLite(document).ready(function(){angularInit(document,bootstrap)}))}(window,document),!window.angular.$$csp()&&window.angular.element(document).find("head").prepend('<style type="text/css">@charset "UTF-8";[ng\\:cloak],[ng-cloak],[data-ng-cloak],[x-ng-cloak],.ng-cloak,.x-ng-cloak,.ng-hide:not(.ng-hide-animate){display:none !important;}ng\\:form{display:block;}</style>'),function(){function getCookie(name){var cookieValue=null;if(document.cookie&&""!=document.cookie)for(var cookies=document.cookie.split(";"),i=0;i<cookies.length;i++){var cookie=jQuery.trim(cookies[i]);if(cookie.substring(0,name.length+1)==name+"="){cookieValue=decodeURIComponent(cookie.substring(name.length+1));break}}return cookieValue}function csrfSafeMethod(method){return/^(GET|HEAD|OPTIONS|TRACE)$/.test(method)}function sameOrigin(url){var host=document.location.host,protocol=document.location.protocol,sr_origin="//"+host,origin=protocol+sr_origin;return url==origin||url.slice(0,origin.length+1)==origin+"/"||url==sr_origin||url.slice(0,sr_origin.length+1)==sr_origin+"/"||!/^(\/\/|http:|https:).*/.test(url)}window.kratosResponse={user:"",users:[],teams:[],log:[]},angular.module("OSWizardApp",[]),angular.module("OSWizardApp").factory("UserService",function(){var user={id:"",name:"",parsedRoles:[],isGHUser:!0},users=[];return{user:user,users:users,isTeamAdmin:function(permissions){return"undefined"!=typeof permissions?permissions.indexOf(this.user.id)>-1:!1},getByID:function(id){var requestedUser;return angular.forEach(this.users,function(user){user.name===id&&(requestedUser=user)}),requestedUser},getName:function(id){return this.getByID(id).data.username},parseRole:function(role){return{resource:role.split("|")[0],role:role.split("|")[1]}}}}),angular.module("OSWizardApp").controller("TeamsCtrl",function($scope,$http,$filter,$element,$timeout,UserService){$scope.loggedIn=!0,$scope.user=UserService.user,$scope.users=UserService.users,$scope.teams=[],$scope.waiting=!1,$scope.confirmMessage={show:!1},$scope.requestURL="/kratos/orgs/devdesign/teams/",$scope.$watch(function(){return UserService.users},function(newValue){$scope.users=newValue}),$scope.ready=function(){return""===$scope.user.id?!1:""===$scope.user.name?!1:0===$scope.users.length?!1:0===$scope.teams.length?!1:($scope.initDataDependantProps(),!0)},$scope.initDataDependantProps=function(){$scope.canAdd=getObj($scope.user,["perms","team","add"]),$scope.canRemove=getObj($scope.user,["perms","team","remove"]),$scope.editable=$scope.canAdd&&$scope.canRemove},$scope.testStatus=function(status){$scope.loggedIn=401===status?!1:!0},$scope.add=function(name){$scope.waiting=!0,$.ajax({type:"PUT",url:$scope.requestURL+name,contentType:"application/json"}).done(function(msg){window.kratosResponse.log.push({done:angular.fromJson(msg)}),$scope.$apply(function(){$element.find("#add-team .slats-type_input").val(""),$scope.teams.push($filter("prepTeamData")([msg])[0]),$scope.waiting=!1,$scope.confirmMessage.show=!0,$scope.confirmMessage.message="Added",$scope.confirmMessage.teamName=msg.name,$scope.confirmMessage.supplement='You will find it in the "Other teams" section.',$timeout(function(){$scope.confirmMessage.show=!1},4e3)})}).error(function(msg){window.kratosResponse.log.push({error:angular.fromJson(msg)}),$scope.$apply(function(){$scope.waiting=!1,$scope.confirmMessage.show=!0,$scope.confirmMessage.message="There was a problem adding",$scope.confirmMessage.teamName=msg.name,$timeout(function(){$scope.confirmMessage.show=!1},4e3)})})},$scope.remove=function(teamToRemove){var assetObj,index;angular.forEach($scope.assets,function(asset){(asset.name===teamToRemove.name||asset["new"]===teamToRemove.name)&&(assetObj=asset)}),index=$scope.assets.indexOf(assetObj),$.ajax({type:"DELETE",url:$scope.requestURL+assetObj.id}).done(function(msg){window.kratosResponse.log.push({done:angular.fromJson(msg)}),$scope.$apply(function(){$scope.assets.splice(index,1),$scope.updateAssets()})}).error(function(msg){window.kratosResponse.log.push({error:angular.fromJson(msg)})})},$http.get("/kratos/user/").success(function(response){var preppedResponse=$filter("prepUserData")(response);angular.copy(preppedResponse,UserService.user),window.kratosResponse.user=UserService.user}).error(function(response,status){$scope.testStatus(status)}),$http.get("/kratos/users/").success(function(response){var preppedResponse=$filter("prepUsersData")(response);angular.copy(preppedResponse,UserService.users),window.kratosResponse.users=UserService.users}).error(function(response,status){$scope.testStatus(status)}),$http.get("/kratos/orgs/devdesign/teams/").success(function(response){var preppedResponse=$filter("prepTeamData")(response);$scope.teams=preppedResponse,window.kratosResponse.teams=preppedResponse}).error(function(response,status){$scope.testStatus(status)})}),angular.module("OSWizardApp").directive("team",function(){return{restrict:"A",scope:{data:"="},templateUrl:"/static/templates/team.html",link:function(scope){scope.$on("userlistUpdated",function(event,data){scope.$broadcast(data.role.toLowerCase()+"UserlistUpdated",data.users)}),scope.$on("assetlistUpdated",function(event,data){scope.$broadcast(data.type.toLowerCase()+"AssetlistUpdated",data.assets)})}}}),angular.module("OSWizardApp").directive("assetbutton",function(){return{restrict:"E",scope:{teamModel:"=",show:"="},controller:function($scope){$scope.toggle=function(show){$scope.teamModel.showAdmin=!1,$scope.teamModel.showMember=!1,$scope.show=!show}},templateUrl:"/static/templates/assetbutton.html",link:function(scope,element,attrs){scope.heading=attrs.heading,scope.assets=getObj(scope.teamModel.rsrcs,["gh","assets"]),scope.total="undefined"==typeof scope.assets?0:scope.assets.length,element.on("click",function(){scope.show?element.parents(".expandable")[0].expand():element.parents(".expandable")[0].collapse()}),scope.$on(scope.heading.toLowerCase()+"AssetlistUpdated",function(event,data){scope.total=data.length})}}}),angular.module("OSWizardApp").directive("userbutton",function(){return{restrict:"E",scope:{teamModel:"=",show:"="},controller:function($scope){$scope.toggle=function(show){var toggledShow=!show;$scope.teamModel.showAdmin=!1,$scope.teamModel.showMember=!1,$scope.teamModel.showRepo=!1,$scope.show=toggledShow}},templateUrl:"/static/templates/userbutton.html",link:function(scope,element,attrs){scope.role=attrs.role,scope.users=getObj(scope.teamModel.roles,[scope.role.toLowerCase(),"members"]),scope.total="undefined"==typeof scope.users?0:scope.users.length,element.on("click",function(){scope.show?element.parents(".expandable")[0].expand():element.parents(".expandable")[0].collapse()}),scope.$on(scope.role.toLowerCase()+"UserlistUpdated",function(event,data){scope.total=data.length})}}}),angular.module("OSWizardApp").directive("userlist",function($compile,$filter,UserService){return{restrict:"A",scope:{filter:"=",teamModel:"=",show:"="},templateUrl:"/static/templates/userlist.html",link:function(scope,element,attrs){scope.listFilter="",scope.role=attrs.role,scope.canAdd=getObj(scope.teamModel.roles,[scope.role.toLowerCase(),"perms","add"]),scope.canRemove=getObj(scope.teamModel.roles,[scope.role.toLowerCase(),"perms","remove"]),scope.editable=scope.canAdd&&scope.canRemove,scope.userIDs=getObj(scope.teamModel.roles,[scope.role.toLowerCase(),"members"]),scope.users=[],scope.showAllUsers=!1,angular.forEach(scope.userIDs,function(value){scope.users.push(UserService.getByID(value))}),scope.requestURL="/kratos/orgs/devdesign/teams/"+scope.teamModel.name+"/roles/"+scope.role.toLowerCase()+"/",scope.updateUsers=function(){scope.allUsers=$filter("removeUsers")(scope.users),angular.forEach(scope.allUsers,function(user){user.showConfirmRemove=!1}),scope.total="undefined"==typeof scope.users?0:scope.users.length,scope.$emit("userlistUpdated",{role:scope.role,users:scope.users})},scope.inUserList=function(user){return scope.users.indexOf(user)>-1},scope.confirmRemove=function(user){user.showConfirmRemove=!0},scope.abortRemove=function(user){user.showConfirmRemove=!1},scope.add=function(user){$.ajax({type:"PUT",url:scope.requestURL+user.name}).done(function(msg){window.kratosResponse.log.push({done:angular.fromJson(msg)}),scope.$apply(function(){scope.users.push(user),scope.updateUsers()})}).error(function(msg){window.kratosResponse.log.push({error:angular.fromJson(msg)})})},scope.remove=function(user){$.ajax({type:"DELETE",url:scope.requestURL+user.name}).done(function(msg){window.kratosResponse.log.push({done:angular.fromJson(msg)}),scope.$apply(function(){var index=scope.users.indexOf(user);scope.users.splice(index,1),scope.updateUsers()})}).error(function(msg){window.kratosResponse.log.push({error:angular.fromJson(msg)})})},scope.updateUsers()}}}),angular.module("OSWizardApp").directive("assetlist",function($filter,$timeout){return{restrict:"A",scope:{teamModel:"=",heading:"="},templateUrl:"/static/templates/assetlist.html",link:function(scope,element,attrs){scope.canAdd=getObj(scope.teamModel.rsrcs,["gh","perms","add"]),scope.canRemove=getObj(scope.teamModel.rsrcs,["gh","perms","remove"]),scope.editable=scope.canAdd&&scope.canRemove,scope.heading=attrs.heading,scope.assets=[],scope.assets=getObj(scope.teamModel.rsrcs,["gh","assets"]),angular.forEach(scope.assets,function(asset){asset["new"]&&(asset.name=asset["new"])}),scope.assets=$filter("orderBy")(scope.assets,"name"),scope.total=getObj(scope,["assets","length"])||0,scope.waiting=!1,scope.confirmMessage={show:!1},scope.requestURL="/kratos/orgs/devdesign/teams/"+scope.teamModel.name+"/resources/gh/",scope.updateAssets=function(){scope.total=getObj(scope,["assets","length"])||0,angular.forEach(scope.assets,function(asset){asset.showConfirmRemove=!1}),scope.$emit("assetlistUpdated",{type:scope.heading,assets:scope.assets})},scope.confirmRemove=function(asset){asset.showConfirmRemove=!0},scope.abortRemove=function(asset){asset.showConfirmRemove=!1},scope.add=function(name){var data={"new":name};scope.waiting=!0,$.ajax({type:"POST",url:scope.requestURL,data:JSON.stringify(data),contentType:"application/json"}).done(function(msg){window.kratosResponse.log.push({done:angular.fromJson(msg)}),scope.$apply(function(){data.name=data["new"],scope.assets.unshift(data),scope.updateAssets(),element.find(".slats-type_input").val(""),scope.waiting=!1,scope.confirmMessage.show=!0,scope.confirmMessage.message="Added",scope.confirmMessage.assetName=data.name,$timeout(function(){scope.confirmMessage.show=!1},4e3)})}).error(function(msg){window.kratosResponse.log.push({error:angular.fromJson(msg)}),scope.$apply(function(){scope.waiting=!1,scope.confirmMessage.show=!0,scope.confirmMessage.message="There was a problem adding",scope.confirmMessage.assetName=data.name,$timeout(function(){scope.confirmMessage.show=!1},4e3)})})},scope.remove=function(assetToRemove){var assetObj,index;angular.forEach(scope.assets,function(asset){(asset.name===assetToRemove.name||asset["new"]===assetToRemove.name)&&(assetObj=asset)}),index=scope.assets.indexOf(assetObj),$.ajax({type:"DELETE",url:scope.requestURL+assetObj.id}).done(function(msg){window.kratosResponse.log.push({done:angular.fromJson(msg)}),scope.$apply(function(){scope.assets.splice(index,1),scope.updateAssets()})}).error(function(msg){window.kratosResponse.log.push({error:angular.fromJson(msg)})})},scope.updateAssets()}}}),angular.module("OSWizardApp").directive("role",function($filter,UserService){return{restrict:"E",scope:{teamModel:"=",username:"="},templateUrl:"/static/templates/role.html",link:function(scope,element){scope.roles=[],angular.forEach(scope.teamModel.roles,function(role,key){var members=getObj(role,["members"])||[];members.indexOf(UserService.user.id)>-1&&scope.roles.push(key)}),scope.roles=$filter("orderBy")(scope.roles),element.addClass("corner-badge corner-badge__user"),element.append('<span class="corner-badge_label">'+scope.roles.join(", ")+"</span>"),-1!==scope.roles.indexOf("admin")&&element.addClass("corner-badge__bg-green"),0===scope.roles.length&&element.addClass("corner-badge__hide")}}}),angular.module("OSWizardApp").directive("expandable",function(){return{restrict:"A",link:function(scope,element){$(element).expandable()}}}),angular.module("OSWizardApp").filter("username",function(UserService){return function(id){return UserService.getName(id)}}),angular.module("OSWizardApp").filter("removeUsers",function(UserService){return function(users){var filteredUsers=[];return angular.forEach(UserService.users,function(user){-1===users.indexOf(user)&&filteredUsers.push(user)}),filteredUsers}}),angular.module("OSWizardApp").filter("hasCurrentUser",function(UserService){return function(teams,toggle){var newTeams=[],inOut=!0;return"undefined"!=typeof toggle&&(inOut=toggle),angular.forEach(teams,function(team){var inTeam=!1;angular.forEach(team.roles,function(role){var members=getObj(role,["members"])||[];members.indexOf(UserService.user.id)>-1&&(inTeam=!0)}),inOut?inTeam&&newTeams.push(team):inTeam||newTeams.push(team)}),newTeams}}),angular.module("OSWizardApp").filter("toArray",function(){return function(obj){var array=[];return angular.forEach(obj,function(obj_prop){array.push(obj_prop)}),array}}),angular.module("OSWizardApp").filter("prepUserData",function(UserService){return function(user){var output={id:user.name,name:user.data.username,parsedRoles:[],isGHUser:!0,perms:{}},isGHUser=!1;return angular.forEach(user.roles,function(role){output.parsedRoles.push(UserService.parseRole(role))}),angular.forEach(output.roles,function(role){"gh"===role.resource&&(isGHUser=!0)}),output.isGHUser=isGHUser,output.perms=user.perms,output}}),angular.module("OSWizardApp").filter("prepUsersData",function(UserService){return function(users){var output=[];return angular.forEach(users,function(user){user.parsedRoles=[],angular.forEach(user.roles,function(role){user.parsedRoles.push(UserService.parseRole(role))}),output.push(user)}),output}}),angular.module("OSWizardApp").filter("prepTeamData",function(){return function(teams){var output=[];return angular.forEach(teams,function(team){team.showAdmin=!1,team.showMember=!1,team.showRepo=!1,output.push(team)}),output}});var csrftoken=getCookie("csrftoken");$.ajaxSetup({beforeSend:function(xhr,settings){!csrfSafeMethod(settings.type)&&sameOrigin(settings.url)&&xhr.setRequestHeader("X-CSRFToken",csrftoken)}});var getObj=function(obj,path){var key=path.shift();return obj=obj[key],obj&&path.length?getObj(obj,path):obj}}();